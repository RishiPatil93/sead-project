#!/usr/bin/env node
// Smoke-test runner (CommonJS) to work in projects with "type": "module"
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.warn('Register failed (maybe user exists):', err.response && err.response.data ? err.response.data : err.message || err);
      try {
        ...
        console.error('Login failed:', loginErr.response && loginErr.response.data ? loginErr.response.data : loginErr.message || loginErr);
        process.exit(3);
      }
    }
